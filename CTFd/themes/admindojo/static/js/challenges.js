...
function loadchal(id) {
    ...
    if (obj.type === 'hidden') {
        ezal({
            title: "Training Hidden!",
            body: "You haven't unlocked this training yet!",
            button: "Got it!"
        });
        return;
    }
    ...
    $.get(script_root + "/api/v1/challenges", function (response) {
        ...
        for (var i = 0; i <= challenges.length - 1; i++) {
            ...
            if (user_solves.indexOf(chalinfo.id) == -1) {
                var chalbutton = $("<button class='btn btn-dark challenge-button w-200 text-truncate pt-3 pb-3 mb-2' value='{0}'></button>".format(chalinfo.id));
            } else {
                var chalbutton = $("<button class='btn btn-dark challenge-button solved-challenge w-200 text-truncate pt-3 pb-3 mb-2' value='{0}'><i class='fas fa-check corner-button-check'></i></button>".format(chalinfo.id));
            }
